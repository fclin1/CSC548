...
/* Everything here is `static inline`: the header is included from more than
 * one translation unit (the MPI build has several). */

static inline void swap_nnzs(coo_matrix *coo, int ind1, int ind2)
{
    int eleind1;
...
/* Deterministic problem data; x must have room for coo->num_cols floats.
 * Values are overwritten because some SuiteSparse matrices carry extremes that
 * make single-precision checking meaningless. x comes from the same stream
 * right after, exactly as spmv.c does it, so both sequential baselines and the
 * MPI harness see identical inputs. */
static inline void fill_problem(coo_matrix *coo, float *x, unsigned int seed)
{
    srand(seed);
    for (int i = 0; i < coo->num_nonzeros; i++)
